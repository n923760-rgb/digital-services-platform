...
The PDF merge conversation is persisted in PostgreSQL but remains **off by default** (`TELEGRAM_ORDERS_ENABLED=false`). The flag alone does not make a production service safe: an enabled registry entry, authorized wallet funding, full document safety checks and operational controls are still required. The Compose deployment now includes a no-network PDF processor container with a private shared volume and resource limits. Never enable public uploads against the bundled development S3 emulator.
